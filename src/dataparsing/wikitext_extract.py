...
def wikitext_to_plaintext(wikitext):
    ...
    plaintext = (
        "_PARAGRAPHSTART_ " + wikitext
    )  # Each segment is its own paragraph, so add tag

    # Bold text:
    quantity = int(len([m.start() for m in re.finditer("'''", plaintext)]) / 2)
    for i in range(0, quantity):
        plaintext = plaintext.replace("'''", "_BOLDSTART_ ", 1).replace(
            "'''", " _BOLDEND_", 1
        )
    ...
    quantity = int(len([m.start() for m in re.finditer("''", plaintext)]) / 2)
    for i in range(0, quantity):
        plaintext = plaintext.replace("''", " _ITALICSTART_ ", 1).replace(
            "''", " _ITALICEND_ ", 1
        )
    ...
    if plaintext.startswith("_PARAGRAPHSTART_ \n="):
        ...
        if header_count % 2 == 0:  # Must have an even number of ='s signs
            ...
            plaintext = plaintext.replace(
                header_text, " _HEADER" + str(header_number) + "START_ ", 1
            )
            plaintext = plaintext.replace(
                header_text, " _HEADER" + str(header_number) + "END_ ", 1
            )
    ...
    if ref_starts and len(ref_starts) == len(ref_ends):
        ...
        for start in ref_starts:
            plaintext = (
                plaintext[: ref_starts[index]] + "" + plaintext[ref_ends[index] + 6 :]
            )
            index += 1
    ...
    for reference in reference_starts:
        ...
        for URL in external_URLs:
            if not URL:
                continue
            if (
                citation_info.find(URL) != -1
            ):  # Check if a known external URL is between <ref> and </ref>
                if_URL_found = True
                external_URL = URL
                ...
                if (
                    label_name_index != -1
                ):  # Citation contains a group name too; the URL can then be used for citations with the same label
                    if (
                        '"' in citation_info
                    ):  # E.g. if the ref template is <ref name="whatever">, remove the quotes
                        citation_group_quote = re.search(
                            double_quote_re, citation_info[label_name_index:]
                        ).group()  # Find first double quote after name=
                        citation_group = citation_group_quote[
                            1:-1
                        ]  # Remove the double quotes (first and last characters)
        ...
        if (
            not if_URL_found and URL
        ):  # No known external URL found in the template; search for a group instead
            label_name_index = citation_info.find("name=")
