...
def correct():
    ...
    if if_session_exists:
        ...
        html_output = correction.correction(article_title,data,input_text,text_quotes,language="en",
             ...
             if_detect_NNP = if_detect_NNP, if_detect_JJ = if_detect_JJ,
...
def article():
    ...
    #Finished checks
    #Submit to backend:
    output = main.main(article_title = filtered_name, language=language, if_ignore_URL_error=True,
                            if_detect_quote=if_quote,if_detect_CD=if_cardinal_number,
                            if_detect_NNP=if_singular_proper_noun,if_detect_NN=if_noun,
    ...
    if(html_output!="500"):
        ...
        return render_template("article.html", text = html_output, page = filtered_name, language = language)
    else: #Generic error
