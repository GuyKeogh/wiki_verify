...
def generate_api_header():
    ...
    header = {
        "User-Agent": "wiki_verify/"
        + __metadata__.__VERSION__
        + "(https://verify.toolforge.org/) "
        + if_from_web_text,
...
def download_article(article_title, language):
    ...
    html_page = (
        html_page.replace("<h2>", " _HEADER2START_ ")
        .replace("</h2>", " _HEADER2END_ ")
        .replace("<h3>", " _HEADER3START_ ")
        .replace("</h3>", " _HEADER3END_ ")
        .replace("<b>", " _BOLDSTART_ ")
        .replace("</b>", " _BOLDEND_ ")
        .replace("<i>", " _ITALICSTART_ ")
        .replace("</i>", " _ITALICEND_ ")
        .replace("<p>", " _PARAGRAPHSTART_ ")
        .replace("</p>", " _PARAGRAPHEND_ ")
    )
...
def download_external_URLs(article_title, language):
    ...
    if __metadata__.__IF_WEB__:
        external_link_limit = (
            __metadata__.__WEB_EXTERNAL_URL_LIMIT__ + 1
        )  # +1 so error can be reported if too many
...
def get_URL_text(URL, headers, if_ignore_URL_error=True):
    ...
    try:
        ...
        try:
            ...
            parsed_html = BeautifulSoup(html_page, "html.parser")
            plain_text = remove_junk(
                parsed_html.find_all(text=True)
            )  # Best method of getting bare text
