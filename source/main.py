    ...
    if (
        ...
        try:  # Download list of external links ( FIXME: get it all from wikitext in future)
            if if_evaluate_citations:
                external_URLs = web_scraper.download_external_URLs(
                    article_title, settings["language"]
                )
                if (
                    len(external_URLs) == 1
                    and external_URLs[0] == "_ERROR: problem getting external_URLs_"
                ):
        ...
        try:  # Download article
            wikitext = web_scraper.download_wikitext(
                article_title, settings["language"]
            )
        ...
        for position in newlines:
            ...
            plaintext = wikitext_extract.wikitext_to_plaintext(
                wikitext[position:position_end]
            )  # Text covering the segment, stripped of refs and templates
                ...
                if not if_in_template:
                    if "{{" in plaintext:
                        ...
                        for citation in citation_data:
                            ...
                            if not data[
                                "if_attach_all_citations"
                            ]:  # Default: citations are only evaluated when they're in same paragraph as text
                        ...
                        text_segments.append(
                            tuple(
                                (position, position_end, plaintext, relevant_citations)
                            )
                        )
        ...
        for wiki_part in text_segments:
            if wiki_part[0] > segment_last and wiki_part[1] <= segment:
                ...
                for cite_URL in wiki_part[3]:
                    if cite_URL in processed_citations:
                        ...
                        if cite_words["text"] == "404":
                            continue
                        tags = text_tagging.compare_citation_and_text_terms(
                            tags,
                            cite_words["CD"],
                            cite_words["JJ"],
                            cite_words["NN"],
                ...
                if settings["quote?"]:
                    ...
                    tags = text_tagging.detect_quotes_in_multiple_texts(
                        tags, compiled_cite_text, text_quotes
                    )
